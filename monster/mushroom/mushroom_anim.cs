...
public class mushroom_anim : MonoBehaviour
{
    ...
    IEnumerator atck() //공격
    {
        ...
        gameObject.GetComponent<SpriteRenderer>().sprite = sprite_temp[3];
        GameObject efect_temp = Instantiate(atck_obj);
        efect_temp.GetComponent<mush_atck_modul>().trun_Manager = monster_data_temp.trun_manager_temp;
        efect_temp.GetComponent<mush_atck_modul>().position_temp = transform.position;
